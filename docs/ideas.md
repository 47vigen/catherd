...
- **Trim the shipped models.dev snapshot.** `catalog/models-dev.json` is 1.4 MB, the full `api.json` minus nothing catherd needs removed. Keep only the fields the capability checks read (tool call, image in/out, reasoning, context) and the providers the user's backends can reach. *Why:* it ships in every `bunx catherd` install. *Where:* `trimModelsDev` in `src/routing/catalog.ts`.
