...
This gates `dispatch`. It needs the user (Vigen) at Claude Desktop, because MCP servers load at session start. Nothing in Tasks 3–13 depends on its outcome except Task 8's blocking behavior and Task 11's "Waiting" section; Task 14 runs only if S1 fails.
...
- [ ] **Step 5: Run the spike in a NEW Claude Desktop session (Vigen)**
...
- [ ] **Step 2: Observe in a NEW Claude Desktop session (Vigen)**
...
- [ ] **Step 3: Check it in a NEW Claude Desktop session (Vigen)**
